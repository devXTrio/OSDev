 ...
 * itself, so we simulate that by calling kmalloc() and free().  We assume that
 ...
 * In the simulated implementation, it just calls the system call kmalloc(),
 ...
 * kmalloc(), since the simulated filesystem hardware lives in actual memory.
 *
