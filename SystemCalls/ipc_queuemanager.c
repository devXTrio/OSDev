...
int enqueue_ipc(enum MESSAGE_QUEUES source, enum MESSAGE_QUEUES destination, char *string) {
    /* Enqueue */
    struct queue_message_t *dest_queue = get_message_ipc(destination);
    struct queue_message_t *source_queue = get_message_ipc(source);
    ...
    struct message *newmessage = find_nonempty_ipc(dest_queue);
    int i = 0;
 ...
 */
void clear_ipc(struct message *m){
    int i = 0;
    m->source = -1;
    m->destination = -1;
    for (i = 0; i < MESSAGE_SIZE; i++) {
...
struct message dequeue_ipc(enum MESSAGE_QUEUES message_queue_enum){
    ...
    struct queue_message_t *queue = get_message_ipc(message_queue_enum);
...
int has_message(enum MESSAGE_QUEUES check){
    ...
    struct queue_message_t *queue = get_message_ipc(check);
